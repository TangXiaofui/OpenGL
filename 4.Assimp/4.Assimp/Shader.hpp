...
public:
    ...
    /**
     ...
     @param uniform 着色器内vec3参数的变量名称
     @param vec 需要传入的vec3参数
     @param useShader 是否使用着色器
     @return true == 传入成功
     */
    bool setUniformVector3f(const char* uniform,vec3 vec,GLboolean useShader = true);
    ...
    /**
     ...
     @param uniform 这色器内GLfloat参数的变量名称
     @param value 需要传入的GLfloat参数
     @param useShader 是否使用着色器
     @return true == 传入成功
     */
    bool setUniform1f(const char *uniform,GLfloat value,GLboolean useShader = true);
    
    
    /**
     向着色器传入GLint值
     
     @param uniform 这色器内GLint参数的变量名称
     @param value 需要传入的GLint参数
     @param useShader 是否使用着色器
     @return true == 传入成功
     */
    bool setUniform1i(const char *uniform,GLint value,GLboolean useShader = true);
